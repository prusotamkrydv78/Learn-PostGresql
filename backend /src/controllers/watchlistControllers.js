...
const createWatchlist = async (req, res) => {
    ...
    if (existing) {
        res.status(402).json({ sucess: true, message: "Movie is already in watchlist" })
    }
    console.log({
        movieId, notes, rating, status: status || "PLANNED", userId: req.user.id
    })
    const addedMovie = await prisma.watchListItem.create({
        data: {
            movieId, notes, rating, status: status || "PLANNED", userId,
...
const updateWatchlist = async (req, res) => {
    ...
    const updateMovie = await prisma.watchListItem.update({
        data: {
            notes, rating, status: status || "PLANNED", userId
        }, where: {
            userId_movieId: {
                movieId, userId
            }
...
const deleteWatchlist = async (req, res) => {
    ...
    if (!watchListItem) {
        return res.status(404).json({ sucess: false, message: "Watchlist item not found" })
    }
